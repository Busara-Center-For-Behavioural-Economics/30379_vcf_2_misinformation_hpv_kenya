...
THINGS TO DO:
- filter by consent after you figure out refusal rates and their discrepancies
- clean the randomised questions and add them to the "arbitrary responses" bit
...
baseline_data %>% 
  ...
  filter(submission_date_dateonly > as_date("2023-11-10")) %>%
  summarise(n()) %>% 
  arrange(en_name)
...
which_idx_are_insufficient <- c(6,9, 18, 133, 135, 220, 222, 365, 395, 450, 456, 457, 591)
...
baseline_data %>% 
  ...
  filter(submission_date_dateonly >= "2023-11-01") %>% 
  group_by(en_name, submission_date_dateonly) %>% 
  summarise(n = n()) %>% 
  arrange(desc(n))
...
#(icc <- clus.rho(popchar = baseline_data$vax[baseline_data$vax_beh_int_1 != 88], cluster = baseline_data$B0_cluster[baseline_data$vax_beh_int_1 != 88]))
...
temp_df <- baseline_data[baseline_data$B1_name %in% duplicate_names,c("starttime", "en_name", "B1_name", "B0_cluster", "county_name", "B7_phonenumber")] %>% 
  filter(starttime > as_date("2023-11-06")) %>% 
...
manual_backchecks <- unique(c(benjamin_thoge, ruth_ndungu_too_quick, ruth_ndungu_additional, duplicate_phonenumber_different_name, ruth_kilobi, evans_otieno))
...
# Given average daily progress, and days left how many FOs need to work in which clusters to equalise sample size
...
temp_df <- baseline_data %>% 
  filter(submission_date_dateonly >= as_date("2023-11-06")) %>% 
  group_by(en_name) %>% 
