...
baseline_data_vax_beh_int_1 <- read.csv("Raw Data/Main Data Collection/VCF2 - Baseline - Vax_Beh_Int_1 old.csv")[-14906,]

# Add one variable lost in surveyCTO
baseline_data <- cbind(baseline_data, vax_beh_int_1 = baseline_data_vax_beh_int_1$vax_beh_int_1)
...
# Add treatment variable for a few buggy responses
baseline_data$cluster_treatment_group[baseline_data$final_ID %in% c("84154Nov18093627", "35128Nov18105612", "33116Nov18114828", "35188Nov18125741", "57117Nov18140126", "43117Nov18151628")] <- "Offline_Only"
baseline_data$cluster_treatment_group[baseline_data$final_ID %in% c("42157Nov13104026", "77114Nov13105220")] <- "Control"
...
baseline_data_new <- baseline_data %>% 
  #filter(cluster_treatment_group %in% c("Offline_Only", "Online_Offline")) %>% 
  group_by(B11a_phonenumber) %>%
  dplyr::mutate(hairdresser_ID = cur_group_id())
...
hairdresser_database <- baseline_data_new_ready_for_hairdresser_database %>% 
  ...
  dplyr::summarise(response_final_IDs = paste(final_ID, collapse = ", "),
            treatment_group = {
