...
We can't afford to move forward with all participants.

#### 1. Fix input errors from FOs

```{r}

# survey  ID 6697B4 was keyed in twice by mistake. Please replace the one done earlier before 3pm to 6696H6.

# In cluster 292 the field officer by the name Bernard Musili accidentally keyed two hairdressers using the same survey ID (7835B1) Discard Veronicah Wanjiru from the data

```

#### 2. Merge and Split Hairdressers

```{r}

# We had a case where Survey ID 6694P0 is being shared by two different respondents who should be independent

# The following are duplicate IDs: 28B8 same person as 4577K1, 4822X2 same as 7588D0, 5976N4 same as 5124Q3


merge_hairdressers <- function(){
