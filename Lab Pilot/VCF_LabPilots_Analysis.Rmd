...
lapply(colnames(truth_discernment), function(i) tblFun(truth_discernment[,i]))

lapply(colnames(truth_discernment), function(i) tblFun_II(truth_discernment[,i], truth_discernment$introduction.1.player.assigned_group))
...
compute_auc <- function(truth_discernment = truth_discernment, true_false_coding = true_false_coding){
  ...
  for(i in 1:nrow(truth_discernment)){
    ...
    for(threshold in 1:5){
      ...
      hit_rate <- sum(ifelse(perceived_binary_coded == "Perceived_False" & true_false_coding == FALSE, 1, 0), na.rm = T)/(16/2)
      false_alarm_rate <- sum(ifelse(perceived_binary_coded == "Perceived_False" & true_false_coding == TRUE, 1, 0), na.rm = T)/(16/2)
    ...
    if(length(unique(false_alarm_rate_all_thresholds)) < 2 | length(unique(hit_rate_all_thresholds)) < 2){
      auc_temp <- NA
    } else{
...
# Create temp df
temp_df <- combined %>% 
  dplyr::select(introduction.1.player.assigned_group, introduction.1.player.full_posts_order, introduction.1.player.bottom_posts_order, starts_with("survey.1.player.detect_misinfo_quiz"))
...
participant_row_split <- str_split(str_remove_all(temp_df$survey.1.player.detect_misinfo_quiz7_2, "\\'|\\]|\\[|[:digit:]"), ",  ")
participant_row_clean <- process_vectors(participant_row_split)


tblFun(sapply(participant_row_clean, function(i) any(i %in% "Discrediting opponents")))
# tblFun(sapply(participant_row_clean[manipulativeness_identification$survey.1.player.detect_misinfo_quiz7_1 == "Yes" & manipulativeness_identification$introduction.1.player.assigned_group %in% "Debunking_Doctor"], function(i) any(i %in% "Discrediting opponents")))
...
# Create temp df
temp_df <- combined %>% 
  dplyr::select(introduction.1.player.assigned_group, introduction.1.player.full_posts_order, introduction.1.player.bottom_posts_order, contains(paste0("survey.1.player.detect_misinfo_quiz", 1:16, "_3")))
...
# Extract the sharing_propensity
sharing_propensity <- temp_df[,paste0("survey.1.player.detect_misinfo_quiz",1:16,"_4")]
sharing_propensity <- data.frame(lapply(sharing_propensity, as.numeric))

# Perceived Manipulativeness
perceived_manipulativeness <- ifelse(temp_df[,paste0("survey.1.player.detect_misinfo_quiz",1:16,"_1")] == "Yes", TRUE, FALSE)
...
TukeyHSD(aov(rowMeans(sharing_propensity) ~ temp_df$introduction.1.player.assigned_group))

# 2. Avg. sharing intentions for actual manipulative posts ⬇️ for Online, Offline & Online_Offline
TukeyHSD(aov(rowMeans(sharing_propensity[,manipulativeness_actual_bool]) ~ temp_df$introduction.1.player.assigned_group))
...
sharing_propensity_temp <- sharing_propensity
sharing_propensity_temp[!perceived_manipulativeness] <- NA
TukeyHSD(aov(rowMeans(sharing_propensity_temp, na.rm = T) ~ temp_df$introduction.1.player.assigned_group))
...
sharing_propensity_temp <- sharing_propensity[,manipulativeness_actual_bool]
sharing_propensity_temp[perceived_manipulativeness[,manipulativeness_actual_bool]] <- NA
TukeyHSD(aov(rowMeans(sharing_propensity_temp, na.rm = T) ~ temp_df$introduction.1.player.assigned_group))
...
# Create temp df
temp_df <- combined %>% 
  dplyr::select(introduction.1.player.assigned_group, survey.1.player.vax_hes_1, survey.1.player.vax_hes_2)
...
# Make 88's into NAs
temp_df[temp_df == "88 I don't know"] <- NA

# Convert Yes/No into 1s and 0s
temp_df[temp_df == "1 Yes"] <- 1
temp_df[temp_df == "2 No"] <- 0
...
# vax_hes_1: Have you ever received or heard negative information about vaccinations?
# vax_hes_2: Do leaders (religious or political leaders, teachers, health care workers) in your community support vaccinations for infants and children?

# 1. Descriptive Stats by treatment group (frequency tables)
tblFun_II(temp_df$survey.1.player.vax_hes_1, temp_df$introduction.1.player.assigned_group)
tblFun_II(temp_df$survey.1.player.vax_hes_2, temp_df$introduction.1.player.assigned_group)

# 2. Test the difference
TukeyHSD(aov(survey.1.player.vax_hes_1 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_2 ~ introduction.1.player.assigned_group, data = temp_df))
...
4. All childhood vaccines offered by the government program in my community are beneficial. New vaccines carry more risks than older vaccines
5. The information I receive about vaccines from the vaccine program is reliable and trustworthy
6. Getting vaccines is a good way to protect my child/children from disease.
7. Generally I do what my doctor or health care provider recommends about vaccines for my child/children
8. I am concerned about serious adverse effects of vaccines
9. My child/children does or do not need vaccines for diseases that are not common anymore
...
# Create temp df
temp_df <- combined %>% 
  dplyr::select(introduction.1.player.assigned_group, paste0("survey.1.player.vax_hes_",3:12))
...
# How big a problem are the 88s? 1.86% - should be fine
tblFun(unlist(temp_df[,-1]))

# Convert Characters into Numerics
temp_df[temp_df == "5 Strongly Agree"] <- 5
temp_df[temp_df == "4 Agree"] <- 4
temp_df[temp_df == "3 Neither agree nor disagree"] <- 3
temp_df[temp_df == "2 Disagree"] <- 2
temp_df[temp_df == "1 Strongly Disagree"] <- 1
temp_df[temp_df == "88 I don't know"] <- NA
temp_df[temp_df == "99 Refuse to answer"] <- NA
...
# Method
# 1. Transform Likert scale to a numeric variable
# 2. Pairwise t.test between C<>Online, C<>Online_Offline, C<>Offline, Online<>Offline, Online<>Online_Offline, Offline<>Online_Offline (all)

# Hypotheses
# 1. Vax_hes_3-6, 8-10 ⬆️ for Online, Offline, Online_Offline
# 2. Vax_hes_7, 11-12 ⬇️ for Online, Offline, Online_Offline
# 3. Vax_hes_7 ➡️ for Online, Offline, Online_Offline

TukeyHSD(aov(survey.1.player.vax_hes_3 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_4 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_5 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_6 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_7 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_8 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_9 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_10 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_11 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_hes_12 ~ introduction.1.player.assigned_group, data = temp_df))

# Secondary Analysis
# 1. Repeat primary analysis by Gender & Religion to understand if the treatment effect is stronger for some religions or genders.

```


# Vaccine Behavioural Intentions

1. Is your daughter vaccinated against HPV?
2. How likely are you to get more information about the HPV vaccine for your child?
3. How likely are you to consider getting the vaccine for your child?
4. How likely are you to try to get the vaccine for your child?
5. How likely are you to actually get the vaccine for your child?
...
# Create temp df
temp_df <- combined %>% 
  dplyr::select(introduction.1.player.assigned_group, paste0("survey.1.player.vax_beh_int_",1:5))
...
# How big a problem are the 88s?
tblFun(unlist(temp_df[,-1]))

# Make 88's into NAs
temp_df[temp_df == "88 Don't know"] <- NA
...
tblFun_II(temp_df$introduction.1.player.assigned_group, temp_df$survey.1.player.vax_beh_int_1)
tblFun_II(temp_df$survey.1.player.vax_beh_int_2, temp_df$introduction.1.player.assigned_group)
tblFun_II(temp_df$survey.1.player.vax_beh_int_3, temp_df$introduction.1.player.assigned_group)
tblFun_II(temp_df$survey.1.player.vax_beh_int_4, temp_df$introduction.1.player.assigned_group)
tblFun_II(temp_df$survey.1.player.vax_beh_int_5, temp_df$introduction.1.player.assigned_group)
...
# Hypotheses
# 1. vax_beh_int_1-5 ⬆️ for Online, Offline, Online_Offline

TukeyHSD(aov(survey.1.player.vax_beh_int_1 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_beh_int_2 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_beh_int_3 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_beh_int_4 ~ introduction.1.player.assigned_group, data = temp_df))
TukeyHSD(aov(survey.1.player.vax_beh_int_5 ~ introduction.1.player.assigned_group, data = temp_df))
